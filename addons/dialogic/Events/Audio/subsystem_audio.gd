...
func _ready() -> void:
	...
	base_music_player.bus = "Music"
	add_child(base_music_player)
	
	base_sound_player.name = "Sound"
	base_sound_player.bus = "Effect"
	add_child(base_sound_player)


## Updates the background music. Will fade out previous music.
func update_music(path:String = '', volume:float = 0.0, audio_bus:String = "Music", fade_time:float = 0.0, loop:bool = true) -> void:
...
func play_sound(path:String, volume:float = 0.0, audio_bus:String = "Effect", loop :bool= false) -> void:
